...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SESSION['user_logged_in'])) {
    ...
    header('Location: ?page=houses&p=' . $current_page);
    exit();
}
...
// Compter le nombre total de maisons
try {
    $stmt = $pdo->prepare("
...
if ($current_page > $total_pages) {
    header('Location: ?page=houses&p=1');
    exit();
}
...
// Récupérer les maisons pour la page actuelle
try {
    $stmt = $pdo->prepare("
    ...
    <section class="section">
        ...
        <?php if (empty($houses)): ?>
            <div style="text-align: center; padding: 40px; background-color: #f8f9fa; border-radius: 10px; margin: 20px;">
                <h3 style="color: #6c757d;">Aucune maison disponible</h3>
                <p style="color: #6c757d; margin-bottom: 20px;">Il n'y a actuellement aucune annonce de maison à afficher.</p>
        ...
        <?php else: ?>
            <div class="annonces-grid">
                <?php foreach ($houses as $house): 
                    $is_favorite = in_array($house['id'], $user_favorites);
                    $can_edit = isset($_SESSION['user_logged_in']) && ($_SESSION['user_role'] === 'admin' || $house['user_id'] == $_SESSION['user_id']);
                ?>
                    <div class="annonce-card">
                        <img src="<?php echo $house['image_url']; ?>" alt="<?php echo $house['title']; ?>" class="annonce-image">
                        <div class="type-badge <?php echo strtolower($house['transaction_type']) === 'rent' ? 'type-rent' : 'type-sale'; ?>">
                            <?php echo $house['transaction_type']; ?>
                        </div>
                        ...
                        <?php if (isset($_SESSION['user_logged_in'])): ?>
                            <div style="position: absolute; top: 10px; left: 10px; display: flex; gap: 5px;">
                                ...
                                <form method="POST" style="display: inline;">
                                    ...
                                    <input type="hidden" name="listing_id" value="<?php echo $house['id']; ?>">
                                ...
                                <?php if ($can_edit): ?>
                                    <a href="?page=edit-listing&id=<?php echo $house['id']; ?>" 
                        ...
                        <div class="annonce-content">
                            <div class="annonce-header">
                                <h3 class="annonce-titre"><?php echo htmlspecialchars($house['title']); ?></h3>
                                <div class="annonce-prix"><?php echo number_format($house['price'], 0, ',', ' '); ?>€</div>
                            </div>
                            <div class="annonce-localisation"><?php echo htmlspecialchars($house['city']); ?></div>
                            <p class="annonce-description"><?php echo htmlspecialchars($house['description']); ?></p>
                            <div style="font-size: 0.8em; color: #666; margin-top: 10px;">
                                Publié par: <?php echo htmlspecialchars($house['owner_email']); ?>
                            </div>
            ...
            <?php if ($total_pages > 1): ?>
                <div class="pagination" style="margin-top: 3rem; display: flex; justify-content: center; align-items: center; gap: 10px;">
                    ...
                    <?php if ($current_page > 1): ?>
                        <a href="?page=houses&p=<?php echo $current_page - 1; ?>" 
                    ...
                    <?php for ($i = $start_page; $i <= $end_page; $i++): ?>
                        ...
                        <?php else: ?>
                            <a href="?page=houses&p=<?php echo $i; ?>" 
                               class="pagination-btn" 
                               style="padding: 10px 15px; background: #f8f9fa; color: #333; text-decoration: none; border-radius: 5px; border: 1px solid #ddd;">
                                <?php echo $i; ?>
                            </a>
                    ...
                    <?php if ($end_page < $total_pages): ?>
                        <?php if ($end_page < $total_pages - 1): ?>
                            <span style="padding: 10px 5px; color: #666;">...</span>
                        <?php endif; ?>
                        <a href="?page=houses&p=<?php echo $total_pages; ?>" 
                           class="pagination-btn" 
                           style="padding: 10px 15px; background: #f8f9fa; color: #333; text-decoration: none; border-radius: 5px; border: 1px solid #ddd;">
